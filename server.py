from fase.seal import CKKSEncoder, Encryptor, Decryptor, Evaluator, Ciphertext, VectorDouble, Plaintext
import numpy as np
import time
...
class SERVER:
	...
	def linear_approx(self, x):
		...
		A = self.encoder.encode(a, self.scale)
		B = self.encoder.encode(b, self.scale)

		#Evaluating a*x
		self.evaluator.mod_switch_to_inplace(A, x.parms_id())
		ans = self.evaluator.multiply_plain(x, A)
	...
	def re_encrypt(self, x):
		self.count += 1
		#x_plain = Plaintext(); x_vec = VectorDouble()

		x_plain = self.decryptor.decrypt(x)
		x_vec = self.encoder.decode(x_plain)
		x_plain = self.encoder.encode(x_vec, self.scale)
		x = self.encryptor.encrypt(x_plain)
	...
	def add_row_elements(self, x, offset=1):
		if offset == 1:
			for i in range(self.logn):
				#temp = Ciphertext()
				temp = self.evaluator.rotate_vector(x, 2**i, self.gal_keys)
				self.evaluator.add_inplace(x, temp)
		else:
			for i in range(int(np.log2(self.incr))):
				#temp = Ciphertext()
				temp = self.evaluator.rotate_vector(x, (2**i)*offset, self.gal_keys)
	...
	def expand_vector(self, x):
		...
		for i in range(int(np.log2(self.vec_len))):
			temp = self.evaluator.rotate_vector(x, (self.slot_count - 2**i), self.gal_keys)
			self.evaluator.add_inplace(x, temp)
	...
	def partial_add(self, x):
		...
		id_vec = VectorDouble(ones)

		#id_plain = Plaintext(); id = Ciphertext()
		id_plain = self.encoder.encode(id_vec, self.scale, )
		id = self.encryptor.encrypt(id_plain)

		#ans_plain = Plaintext(); ans = Ciphertext()
		ans_plain = self.encoder.encode([0.0], self.scale)
		ans = self.encryptor.encrypt(ans_plain)
		...
		for i in range(int(np.log2(self.vec_len))-1, -1, -1):
			...
			temp = self.evaluator.rotate_vector(id, self.slot_count - 2**i, self.gal_keys)

			s1 = self.evaluator.multiply(temp, x)
	...
	def inv_norm_approx(self, norm):
		...
		div1 = self.encoder.encode([-0.5], self.scale)
		div2 = self.encoder.encode([1.5], self.scale)
		...
		for i in range(r):
			...
			#x_i**2
			#square_x = Ciphertext()
			square_x = self.evaluator.square(guess)
			...
			out_x = self.evaluator.multiply(guess, x)
			...
			sum_x = self.evaluator.multiply_plain(guess, div2)
			...
			guess = self.evaluator.add(sum_x, square_x) # Overwrite?? 
	...
	def goldschmidt(self, X):
		...
		#y = Ciphertext(); 
        # x = Ciphertext(); r = Ciphertext(); h = Ciphertext()
		...
		for i in range(k):
			# print("Iteration :", i)
			neg_one = self.encoder.encode([-1.0], self.scale)
			half = self.encoder.encode([0.5], self.scale)
			...
			#temp_r = x*h
			temp_r = self.evaluator.multiply(x, h)
			...
			r = self.evaluator.add_plain(temp_r, half)

			##x = x + x*r
			#temp_x = Ciphertext()
			self.evaluator.mod_switch_to_inplace(x, r.parms_id())
			temp_x = self.evaluator.multiply(x, r)
			...
			temp_h = self.evaluator.multiply(h, r)
		...
		#two = Plaintext()
		two = self.encoder.encode([2.0], self.scale)
	...
	def init_vector(self):
		...
		temp_vec = VectorDouble(init_vec)
		return temp_vec

	def vect_mat_product(self, X_j, r):
		#x = Ciphertext()
		x = self.evaluator.multiply(X_j, r)
		self.evaluator.relinearize_inplace(x, self.relin_keys)
		self.evaluator.rescale_to_next_inplace(x)
	...
	def power_iteration(self, X):
		...
		for d in range(self.dims):
			...
			r_plain = self.encoder.encode(temp_vec, self.scale)
			r = self.encryptor.encrypt(r_plain)
			eig_val = 0
			for k in range(iterations):

				#S_plain = Plaintext(); S1 = Ciphertext(); S2 = Ciphertext() 
				S_plain = self.encoder.encode([0.0], self.scale)
				S2 = self.encryptor.encrypt(S_plain, )
				S1 = self.encryptor.encrypt(S_plain, )
				...
				#one = Plaintext()
				one = self.encoder.encode([-1.0], self.scale)
